        ...
        (coderes: {
          ...
          axios
            ...
            .then((idres: { data: { id: number } }) => {
              auth
                .getUserIdByToken(req.cookies.token)
                .then((user_id: number) => {
                  pool.query(
                    "REPLACE INTO `spotify` VALUES(?,?,?,?);",
                    [idres.data.id, access_token, refresh_token, user_id],
                    (error) => {
                      if (error) {
                        Sentry.captureException(error);
                        logger.error({ error: error });
        ...
        axios
          ...
          .then((player: currentlyPlaying) => {
            ...
            if (progress < duration / 4) {
              setTimeout(playBackNotification, duration / 4 - progress + 1000);
              logger.debug(
              ...
              return;
            } else if (progress >= duration / 4 && progress < duration / 1.25) {
              setTimeout(
                playBackNotification,
              ...
              setTimeout(playBackNotification, duration - progress + 5000);
            }
            logger.info(
