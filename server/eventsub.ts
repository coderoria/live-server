...
router.post("/eventsub", (req: Request, res: Response) => {
  ...
  if (req.header("Twitch-Eventsub-Message-Type") === "notification") {
    ...
    let requestId: string = req.header("Twitch-Eventsub-Message-Id") as string;
    if (receivedIds.includes(requestId)) {
      res.sendStatus(200);
      logger.debug({ requestId: requestId }, `Notification was a duplicate`);
      return;
    }
        ...
        auth.getUserIdByName(process.env.CHANNEL, (user_id: number) => {
          wantedSubs.forEach((type) => {
            ...
            axios
              ...
              .then(() => {
                logger.debug({ type: type }, "Registered EventSub");
              })
              .catch((error: object) => {
...
function checkSignature(req: Request) {
  ...
  let messageId: string | undefined = req.header("Twitch-Eventsub-Message-Id");
