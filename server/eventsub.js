...
function createSubs() {
    ...
    auth.authSystem(() => {
        auth.getSystemAuth(access_token => {
            pool.query("SELECT `user_id` FROM `admins` WHERE `username`=?;",
                process.env.CHANNEL, (error, res) => {
                    if (error) {
                        console.error(error);
                        return;
                    }
...
function checkSignature(req) {
    ...
    hmac.update(req.header("Twitch-Eventsub-Message-Id") + req.header("Twitch-Eventsub-Message-Timestamp") + JSON.stringify(req.body));
