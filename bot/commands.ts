...
function executeUptime(bot: Client, matches: string[], userstate: Userstate) {
  ...
  twitch.getActiveStreamByName(
    ...
    (data: { started_at: string }) => {
      ...
      let uptime =
        diff.hours() + "h" + diff.minutes() + "min" + diff.seconds() + "sec";
...
function executeQuotes(bot: Client, matches: string[], userstate: Userstate) {
  let newQuote = "";
  if (matches.shift() === "add" && hasPermission(userstate, Level.moderator)) {
    ...
    pool.query(
      ...
      (error: MysqlError | null) => {
        ...
        bot.say(channel, t("commands.quotes.newAdded", { newQuote: newQuote }));
      }
    );
    return;
  }
