  ...
  (error, dbres: OkPacket) => {
    ...
    if (dbres.warningCount == 0) {
      auth.authSystem((success: boolean) => {
        if (!success) return;
        auth.addUser(process.env.CHANNEL as string, (success: boolean) => {
          if (!success) {
            logger.error(
              "Could not add " + process.env.CHANNEL + " as invited user."
            );
            return;
          }
          logger.info(process.env.CHANNEL + " added as invited user.");
        });
      });
    }
  }
